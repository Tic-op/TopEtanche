permissionset 50000 GeneratedPermission
{
    Assignable = true;
    Permissions = tabledata ContactApprobation = RIMD,
        tabledata "Détails Article" = RIMD,
        tabledata "Fiche matériaux" = RIMD,
        tabledata HistVenteArticle = RIMD,
        tabledata "Importation offre prix" = RIMD,
        tabledata "Indicateur Préparation" = RIMD,
        tabledata "Item Distribution" = RIMD,
        tabledata "Item Identifier TICOP" = RIMD,
        tabledata "Item stock min by location" = RIMD,
        tabledata "Ligne préparation" = RIMD,
        tabledata "Logistic resource" = RIMD,
        tabledata "Offre de prix " = RIMD,
        tabledata "Ordre de preparation" = RIMD,
        tabledata "Paramêtre marge" = RIMD,
        tabledata "Propriété matériaux" = RIMD,
        tabledata PurchasePlanning = RIMD,
        tabledata "Reception Entry" = RIMD,
        table ContactApprobation = X,
        table "Détails Article" = X,
        table "Fiche matériaux" = X,
        table HistVenteArticle = X,
        table "Importation offre prix" = X,
        table "Indicateur Préparation" = X,
        table "Item Distribution" = X,
        table "Item Identifier TICOP" = X,
        table "Item stock min by location" = X,
        table "Ligne préparation" = X,
        table "Logistic resource" = X,
        table "Offre de prix " = X,
        table "Ordre de preparation" = X,
        table "Paramêtre marge" = X,
        table "Propriété matériaux" = X,
        table PurchasePlanning = X,
        table "Reception Entry" = X,
        report AvoirVente = X,
        report BL = X,
        report "BonPréparation" = X,
        report "ClientMinimumàFacturer" = X,
        report DevisReport = X,
        report "Dispatch Sales Order Lines" = X,
        report "Dispatch Transfer Lines" = X,
        report "Extraire article" = X,
        report "facture non validée" = X,
        report FactureVente = X,
        report "Gén. Cmde depuis Confirmation" = X,
        report TicketCommandeVente = X,
        codeunit "Attribut management" = X,
        codeunit CostPriceProfitCalculation = X,
        codeunit FindItemEMP = X,
        codeunit FindItemRec = X,
        codeunit "Lot Management" = X,
        codeunit "Montant Toute Lettres" = X,
        codeunit "Planning managment" = X,
        codeunit "PréparationEvent" = X,
        codeunit "Purch Blanket Order From Quote" = X,
        codeunit "Purchase Order Generator" = X,
        codeunit SalesBlanketOrderFromQuote = X,
        codeunit SalesEvents = X,
        codeunit "TICOP TOP-ETANCH" = X,
        codeunit VerificationStock = X,
        page AddDelEmp = X,
        page "Bon de preparation" = X,
        page ContactApprobation = X,
        page "Disponibilité article/mag" = X,
        page "Disponibilité par magasin Démo" = X,
        page "Détail vente article" = X,
        page Emplacement = X,
        page EmplacementsAPI = X,
        page ExportScannedItemR = X,
        page "Fiche matériaux" = X,
        page FindItems = X,
        page HistVenteArticleSubform = X,
        page "Importation offre de prix" = X,
        page InsertReceptionEntry = X,
        page InsertReceptionLine = X,
        page "Item identifier Ticop List" = X,
        page "item list attribut sort" = X,
        page ItemByEmp = X,
        page itemdistribution = X,
        page "KPI Préparation" = X,
        page "Lignes Commande" = X,
        page "Lignes préparations List" = X,
        page "Lignes préparations Subform" = X,
        page "Lignes transfert" = X,
        page ListCommandes = X,
        page "Liste bon de préparation" = X,
        page "Liste des matériaux" = X,
        page "Liste lignes commandes cadres" = X,
        page Listedesarticles = X,
        page Listedesordresdepreparation = X,
        page ListItemByEmp = X,
        page LocationPart = X,
        page loginAPI = X,
        page "Logistic resources list" = X,
        page Magasin = X,
        page magasins = X,
        page MagasinsList = X,
        page ModifOrdrePrep = X,
        page NiveauEmplacement = X,
        page "Offre de prix" = X,
        page "Paramêtres marge" = X,
        page PriceListSubform = X,
        page "Propriété matériaux" = X,
        page "PurchasePlanning List" = X,
        page ReceptionEntries = X,
        page Reclassement = X,
        page "Recommandation Achats" = X,
        page salesLine = X,
        page ScanCode = X,
        page "Select Attribute Dialog" = X,
        page "Stock min d'un article par mag" = X,
        page "Valeur Stock" = X,
        page xxxdeleteItem = X,
        page "XXXX Yasser" = X,
        tabledata "Item Attributes Buffer" = RIMD,
        table "Item Attributes Buffer" = X,
        report BLPageComplete = X,
        codeunit FindItem_Prep = X,
        page "Affectation type" = X,
        page "Affectation Valeur attribut" = X,
        page "Lignepréparation" = X,
        page listeattribut = X,
        report CommandePageComplete = X,
        report DevisPageComplete = X,
        report FacturePageComplete = X,
        tabledata "Retenue à la source" = RIMD,
        table "Retenue à la source" = X,
        report "BLTop-E" = X,
        codeunit FinanceEvents = X,
        page "Retenue à la source Card" = X,
        report AvoirTop = X,
        report RetourTopE = X,
        report "Transfer Shipment Report " = X,
        tabledata DroitDouaneLedgerEntry = RIMD,
        tabledata "Import Folder" = RIMD,
        table DroitDouaneLedgerEntry = X,
        table "Import Folder" = X,
        report "Synthèse Importation" = X,
        codeunit PurchaseEvents = X,
        page "Droit douane ledger entry" = X,
        page "Import folder card" = X,
        page "Import Folder List" = X,
        page "Mettre à jour le document" = X,
        page "Statistic Import Folder" = X,
        report "Transfer Report" = X,
        report "TransferShipReport Complete" = X,
        page "Recherche article" = X,
        page "Usual Search" = X,
        tabledata "Customer VAT Suspension" = RIMD,
        tabledata "Ligne DocVente Regroupée" = RIMD,
        table "Customer VAT Suspension" = X,
        table "Ligne DocVente Regroupée" = X,
        report "BL RGRP" = X,
        codeunit "Sales Document Line Mngmt" = X,
        page "Customer VAT Suspension List" = X,
        report "Feuille préparation" = X,
        report TicketsRetour = X,
        page "Fiche Paiement" = X,
        tabledata "Indicateur TICOP" = RIMD,
        table "Indicateur TICOP" = X,
        report "Bordereau Envoi à la banque" = X,
        report "Caisse journalière" = X,
        report "Disponibilité commande cadre" = X,
        report "Impression Effet Fournisseur" = X,
        report "Ordre de paiement " = X,
        report "Relevé Client " = X,
        report "Retenue Client" = X,
        report "Retenue Fournisseur" = X,
        report "Versement Espece" = X,
        codeunit "ILE UPDATE JOB " = X,
        page "Fiche Paiement BL" = X,
        page "Manager Activities" = X,
        page "Usual search Purchase" = X,
        page "Usual Search Transfer" = X,
        page "modif PL" = X,
        page ModifierDateEcheance = X,
        page "Usual Search Item" = X,
        report Apurement = X,
        report "Devisarchivé" = X,
        report "Importer Qté Achat" = X,
        report "Mise à jour magasin" = X,
        report "Etat de recouvrement/secteur" = X,
        codeunit UpdateSuspensionCondition = X,
        report FactureVenteGRP = X,
        codeunit "Purch Rcpt Pricing Mgt" = X,
        page "Item Active Price " = X,
        page "Item Price Management Card" = X,
        page "Item Sales Price FactBox" = X,
        page "Purch Rcpt Pricing Worksheet" = X,
        report "Calcul des prix de vente" = X,
        codeunit "Init Revaluation Inventory" = X,
        report "Situation commande client" = X,
        codeunit "BO Extraction Handler" = X,
        page "Dialog BO Extraction" = X,
        page "Modif PH" = X,
        page ModifCLE = X,
        report "Mise à jours qté restante" = X,
        page XXXModifFactureNAme = X,
        page XXXXDVLE = X,
        page "xxxxG/L Entry" = X,
        page "xxxxPurch. Inv. Line" = X,
        page xxxxVLE = X,
        page "xxxxxVAT Entry" = X,
        page XXXXXVE = X,
        page "xxxxxxPurch. Inv. Header" = X,
        codeunit "Item Search Engine" = X,
        tabledata "Item Search Buffer" = RIMD,
        tabledata "Temp Item Search" = RIMD,
        table "Item Search Buffer" = X,
        table "Temp Item Search" = X,
        codeunit "Item Search API" = X,
        page "Insert BarCode" = X,
        tabledata Marque = RIMD,
        table Marque = X,
        page "Marque List" = X,
        page Purchaselinexx = X;
}