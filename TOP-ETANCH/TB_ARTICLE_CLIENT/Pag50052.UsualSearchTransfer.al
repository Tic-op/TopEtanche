...
page 50052 "Usual Search Transfer"
{
    ApplicationArea = All;
    Caption = 'Usual Search Transfer';
    PageType = Worksheet;
    SourceTable = item;
   ...
   */
    procedure initvarTransfer(noDoc: Code[20])
    var
    ...
    begin
        TransferH.get(noDoc);
        if TransferH.Status <> TransferH.Status::Open
        then
            Error('Le document doit être ouvert!');
        TransferHNo := noDoc;
    ...
    begin
        ...
        if ItemRec.FindSet() then
            repeat
                ...
                rec.Availability := rec."CalcDisponibilitéWithResetFilters"(TransferH."Transfer-from Code", '');
