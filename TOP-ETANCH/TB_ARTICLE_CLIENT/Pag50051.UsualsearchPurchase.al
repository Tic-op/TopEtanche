...
page 50051 "Usual search Purchase"
{
    ApplicationArea = All;
    Caption = 'Usual search Purchase';
    PageType = Worksheet;
    SourceTable = item;
    UsageCategory = Tasks;
    InsertAllowed = false;
   ...
   */
    ...
    begin
        PurchaseHeader.get(TypeDoc, noDoc);
        if PurchaseHeader.Status <> PurchaseHeader.Status::Open
        then
            Error('Le document doit être ouvert!');
        PurchaseOrderNo := noDoc;
        PurchType := TypeDoc;
        panierVisible := false;
  ...
  */
    ...
    var
        ...
        Purchtype: enum "Purchase Document Type";
