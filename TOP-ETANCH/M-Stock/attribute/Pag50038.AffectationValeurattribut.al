...
page 50038 "Affectation Valeur attribut"
{
        ...
        area(Processing)
        {
            Action(Valider)
            {
                ...
                begin
                    ...
                    If confirm('Cette action va affecter %1 article, veuillez confirmer pour appliquer ces changement', false, rec.count) then begin
                        ...
                        if AffectationParArticle then begin
                            rec.FindFirst();
                            repeat
                                if (DynCaption[1] <> '') and (rec.DynAttr1 <> '') then
    ...
    /*  Trigger OnOpenPage()
     var
         itemrealrec: record item;
     begin
         rec := itemrealrec;  ///loop with transfer fields !! ==> Temporary page
         rec.Insert()
     end; */
    ...
    begin
        ...
        Case ia.Type of
            Ia.type::Date:
                if not Evaluate(vardate, AttributeValue) then
    ...
    var
        famille, catégory, "type", Produit, Matériau : code[20];
